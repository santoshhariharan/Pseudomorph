...
%     jj = getDensityBasedSampling(grpData,distanceTypeForSampling
    jj  = true(sum(ii),1);
    [ indx ] = phenograph(grpData(jj,:),k,'graphtype','Jaccard');
    uInd = unique(indx);
    for j = 1:numel(uInd)
...
% Visualize data
% [~,pcaRedData] = princomp(mCent);
pcaRedData = compute_mapping(mCent,'t-SNE',2);
% pcaRedData = pcaRedData(:,1:3);
figure; hold on;
for i = 1:numel(uG)
    ii = mGrp == i;
%     plot3(pcaRedData(ii,1),pcaRedData(ii,2),pcaRedData(ii,3),...
%         'o','MarkerFacecolor',maps(i,:),'MarkerEdgeColor','none');
    plot(pcaRedData(ii,1),pcaRedData(ii,2),...
        'o','MarkerFacecolor',maps(i,:),'MarkerEdgeColor','none');
end
hold off;
...
uIndx = unique(indxN);
nMap = jet(numel(uIndx));
figure; hold on;
for i = 1:numel(uIndx)
    ii = indxN == uIndx(i);
%     plot3(pcaRedData(ii,1),pcaRedData(ii,2),pcaRedData(ii,3),...
%         'o','MarkerFacecolor',nMap(i,:),'MarkerEdgeColor','none');
    plot(pcaRedData(ii,1),pcaRedData(ii,2),...
        'o','MarkerFacecolor',nMap(i,:),'MarkerEdgeColor','none');
end
hold off;
title('Level-1 Centroids Clustered');
% legend(uControls);
